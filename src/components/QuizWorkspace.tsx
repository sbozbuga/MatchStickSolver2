...
export const QuizWorkspace: React.FC<QuizWorkspaceProps> = ({
  ...
  const handlePointerDown = (
    ...
    const isPresent =
      patterns[charIndex] && patterns[charIndex][segmentIndex] === 1;
  ...
  const handlePointerUp = useCallback(() => {
    if (isDraggingRef.current && dragSource) {
      if (hoverTarget && hasMovedRef.current) {
        ...
        setPatterns((prev) => {
          const newPatterns = [...prev];
          newPatterns[hoverTarget.charIndex] = [
            ...newPatterns[hoverTarget.charIndex],
          ];
        ...
        setPatterns((prev) => {
          const newPatterns = [...prev];
          newPatterns[dragSource.charIndex] = [
            ...newPatterns[dragSource.charIndex],
          ];
  ...
  const renderInteractiveStickDisplay = (
    charIndex: number,
  ...
  return (
    <div
      className="w-full max-w-4xl mx-auto bg-slate-800 rounded-xl p-6 shadow-xl border border-slate-700"
      ref={containerRef}
    >
      ...
      <p className="text-slate-300 mt-2 text-center mb-8 text-lg">
      ...
      <div className="mt-8 flex flex-col items-center gap-4 min-h-[80px]">
        {isSolved ? (
          <>
            <span className="text-3xl font-bold text-emerald-400 animate-bounce">
              Correct! Well done!
            </span>
            <button
          ...
          <>
            <span className="text-3xl font-bold text-red-400 animate-bounce">
              Incorrect!
            </span>
            <button
              onClick={handleReset}
          ...
          <div className="flex gap-4">
            <button
              ...
              className={`px-6 py-2 font-medium rounded-lg border transition flex items-center gap-2 ${
                isCopied

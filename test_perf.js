...
const ALL_PUZZLES_ORIGINAL = new Set();
                                        ...
                                        try {
                                            ...
                                            if (left && right) {
                                                ...
                                                if (leftVal !== null && rightVal !== null && leftVal !== rightVal) {
                                                    ALL_PUZZLES_ORIGINAL.add(testEq);
                                                }
                                            }
                                        } catch (e) {}
                                    }
...
const ALL_PUZZLES_OPT = new Set();
function runOptimized() {
    ALL_PUZZLES_OPT.clear();
    for (const eq of validEquations) {
        const chars = getEquationChars(eq, false);
        const patterns = chars.map(c => [...getPattern(c)]);
        const testChars = patterns.map((p, idx) => patternToChar(p, chars[idx]));
                                ...
                                if (nullCount === 0) {
                                    ...
                                    if (!isEq) {
                                        ...
                                        if (eqIdx > 0 && eqIdx < testChars.length - 1) {
                                            ...
                                            if (leftVal !== null && rightVal !== null && leftVal !== rightVal) {
                                                ALL_PUZZLES_OPT.add(testChars.join(''));
                                            }
                                        }
                                    }
                                }
...
console.log("\n--- Results ---");
console.log(`Original testEq tracking time: ${origTime.toFixed(2)} ms`);
console.log(`Bolt optimized testEq time: ${optTime.toFixed(2)} ms`);
if (optTime < origTime) {
